...
class TennisDataModule(L.LightningDataModule):
    def __init__(self, root, frame_in, is_sequential, train_transform = None, test_transform = None, r = 2.5, w = 512, h = 288, batch_size = 2, num_workers = 2, shuffle = True):
        super().__init__()
        self.root = Path(root)
    ...
    def setup(self, stage: str):
        if stage == 'fit':
            self.train_data = Tennis(root = self.root, train = True, frame_in = self.frame_in, is_sequential = self.is_sequential, transform = self.train_transform, r = self.r, w = self.w, h = self.h)
            self.val_data = Tennis(root = self.root, train = False, frame_in = self.frame_in, is_sequential = self.is_sequential, transform = self.test_transform, r = self.r, w = self.w, h = self.h)

        if stage == 'test':
            self.test_data = Tennis(root = self.root, train = False, frame_in = self.frame_in, is_sequential = self.is_sequential, transform = self.test_transform, r = self.r, w = self.w, h = self.h)
